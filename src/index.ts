...
io.on('connection', (socket) => {
  ...
  socket.on('join_collaboration', (userData) => {
    collaborationUsers.set(socket.id, { id: socket.id, name: userData.name });
    io.emit('collaboration_users_update', Array.from(collaborationUsers.values()));
  });
...
server.listen(Number(PORT), '0.0.0.0', () => {
  console.log(`🚀 Server is running on port ${PORT} (mapped to all interfaces)`);
});
